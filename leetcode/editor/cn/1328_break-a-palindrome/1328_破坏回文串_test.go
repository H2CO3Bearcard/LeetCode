...
func TestBreakAPalindrome(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.Case, func(t *testing.T) {
			if output := breakPalindrome(c.Input); output != c.Output {
...
func breakPalindrome(palindrome string) string {
	...
	palindromeByte := []byte(palindrome)
	for i := 0; i*2+1 < n; i++ {
		if palindromeByte[i] != 'a' {
			palindromeByte[i] = 'a'
			return string(palindromeByte)
		}
	}
	palindromeByte[n-1] = 'b'
	return string(palindromeByte)
}
